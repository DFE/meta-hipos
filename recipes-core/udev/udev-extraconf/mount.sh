...
automount_systemd() {
    ...
    if [ -f /run/systemd/transient/run-media-$name.mount ]; then
        ...
        if /bin/systemctl is-active run-media-$name.mount ; then
            return
        fi
        logger "mount.sh/automount" "/run/media/$name reset unclear service HYP-22728"
	/bin/systemctl reset-failed run-media-$name.mount
    fi
