If you want to build the driver from source code, please copy or download the content of this folder into your home directory. Open a terminal and change your path into the source codes folder.
Use sudo su or su command to work as root user.

Type "make" to build the V4L2 driver from source code.

NOTE #1:
(For audio support, type "make audio=y".  It is recommended to run
 without audio for maximum efficiency and resource usage, 
 if audio is not required.  The default is without audio support)
...
ALSA support (if using audio see NOTE #1 above at top):
