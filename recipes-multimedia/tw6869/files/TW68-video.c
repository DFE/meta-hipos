...
int TW68_g_ctrl_internal(struct TW68_dev *dev, struct TW68_fh *fh, struct v4l2_control *c)
{
        ...
        case V4L2_CID_BRIGHTNESS:
                ...
                c->value = dev->vc[nId].video_param.ctl_bright = regval;
                break;
        case V4L2_CID_HUE:  ///-128 +127
                ...
                if (regval < 0x80)
                        c->value = dev->vc[nId].video_param.ctl_hue = regval;
                else
                        c->value = dev->vc[nId].video_param.ctl_hue = (regval -0x100); 
                break;
        case V4L2_CID_CONTRAST:
                ...
                c->value = dev->vc[nId].video_param.ctl_contrast = regval;
                break;
        case V4L2_CID_SATURATION:
                ...
                c->value = dev->vc[nId].video_param.ctl_saturation = regval /2;
                break;
...
int TW68_s_ctrl_internal(struct TW68_dev *dev,  struct TW68_fh *fh, struct v4l2_control *c)
{
        ...
        switch (c->id) {
        case V4L2_CID_BRIGHTNESS:
                dev->vc[nId].video_param.ctl_bright = c->value;
...
// pci_alloc_consistent   32 4 * 8  continuous field memory buffer
        ...
        for (k=0; k<8; k++) {
                ...
                if (k<4) {
                        dev->vc[k].video_param.ctl_bright = reg_readl(CH1_BRIGHTNESS_REG + k *0x10);
                        ...
                        dev->vc[k].video_param.ctl_bright     = reg_readl(CH1_BRIGHTNESS_REG + (k-4) *0x10 +0x100);
                ...
                printk("TW68_  _video_init1[%d] def AMP: BR %d  CONT %d  HUE_ %d  SAT_U_%d SAT_V_%d   \n", k, 
                       dev->vc[k].video_param.ctl_bright,
	...
	for (k=7; k>0; k--) {
            dev->vc[k].video_param.ctl_bright = dev->vc[k-1].video_param.ctl_bright;
            dev->vc[k].video_param.ctl_contrast = dev->vc[k-1].video_param.ctl_contrast;
            dev->vc[k].video_param.ctl_hue = dev->vc[k-1].video_param.ctl_hue;   
            dev->vc[k].video_param.ctl_saturation = dev->vc[k-1].video_param.ctl_saturation;
            dev->vc[k].video_param.ctl_mute = dev->vc[k-1].video_param.ctl_mute;
            printk("TW68_  _video_init1   get decoder %d default AMP: BRIGHTNESS %d  CONTRAST %d  HUE_ %d  SAT_U_%d SAT_V_%d   \n", k, 
                   dev->vc[k].video_param.ctl_bright, dev->vc[k].video_param.ctl_contrast, dev->vc[k].video_param.ctl_hue, dev->vc[k].video_param.ctl_saturation, dev->vc[k].video_param.ctl_mute);
	}

        // Normalize the reg value to standard value range
        for (k=0; k<8; k++) {
                dev->vc[k].video_param.ctl_bright = (dev->vc[k].video_param.ctl_bright + 0x80) & 0xFF;
                dev->vc[k].video_param.ctl_contrast = dev->vc[k].video_param.ctl_contrast & 0x7F;
                dev->vc[k].video_param.ctl_hue = dev->vc[k].video_param.ctl_hue & 0xFF;   
                dev->vc[k].video_param.ctl_saturation = dev->vc[k].video_param.ctl_saturation & 0xFF;
                dev->vc[k].video_param.ctl_mute = dev->vc[k].video_param.ctl_mute & 0xFF;
		printk("TW68_  _video_init1   remap  %d def AMP: BR %d  CONT %d  HUE_ %d  SAT_U_%d SAT_V_%d   \n", k,
               dev->vc[k].video_param.ctl_bright, dev->vc[k].video_param.ctl_contrast,
               dev->vc[k].video_param.ctl_hue, dev->vc[k].video_param.ctl_saturation,
               dev->vc[k].video_param.ctl_mute);
        }
        return 0;
}
